...
image = Image.open('C:\\Users\\neela\\OneDrive\\Documents\\SnowGOV-main\\Hackathon\\SnowGov.png')
...
def database_management():
    ...
    if choose == 'SCHEMA':
        ...
        if st.button('CREATE'):
            CREATE = f"""<button style="width: 50%; height: 35px; margin-top:20px; background: linear-gradient(to right, #a02a41 0%,    #1D4077 100%); color: white; border-radius: 15px;">CREATE</button>"""
            set_role(conn, "ACCOUNTADMIN")
...
def user_creation_page():
    not_required = option_menu(
        ...
        menu_icon ='person-fill-add',
         styles={
        "container": {"padding": "0!important", "background-color": "#fafafa"},
        "nav-link": {"font-size": "15px", "text-align": "left", "margin":"0px", "--hover-color": "#4e0079"},
        "nav-link-selected": {"background-color": "#4e0079"},
    }
    )
    ...
    if st.button("CREATE"):
        ...
        <button style="width: 50%; height: 35px; margin-top:20px; background: linear-gradient(to right, #a02a41 0%,    #1D4077 100%); color: white; border-radius: 15px;">CREATE</button>
...
def Menu_navigator():
    people_fill=Image.open("C:\\Users\\neela\\OneDrive\\Documents\\SnowGOV-main\\Hackathon\\Icons\\User-Title.png")

    with st.sidebar:
        choice = option_menu(
           menu_title="MENU",
            options=["USER","DATABASE" ,"ROLE", "MONITOR","ABOUT"],
            icons=["people_fill","database-fill", "person-lines-fill", "tv-fill","info-circle-fill"],
